...
let rec compareTypeExpression (te1: TypeExpression, te2) =
    ...
    | Product typeExpr1, Product typeExpr2 -> List.zip typeExpr1 typeExpr2 |> List.iter compareTypeExpression
...
let compareIdentifier (id1, id2) =
    ...
    | IGeneric((s1, _), typings1), IGeneric((s2, _), typings2) ->
        ...
        List.iter compareTyping (List.zip typings1 typings2)
...
let compareTypeDefinition (((id1, _), td1), ((id2, _), td2)) =
    ...
    | Union ids1, Union ids2 ->
        ...
        List.zip ids1 ids2 |> List.iter compareIds
...
and compareAccessor (a1: Accessor, a2) =
    ...
    | AGeneric((s1, _), valueExpr1), AGeneric((s2, _), valueExpr2) ->
        Assert.AreEqual(s1, s2)
        List.zip valueExpr1 valueExpr2 |> List.iter compareValueExpression
...
let compareValueDeclaration (vd1, vd2) =
    ...
    | GenericValue(id1, typings1, typeExpr1), GenericValue(id2, typings2, typeExpr2) ->
        compareIdentifier( id1, id2)
        List.zip typings1 typings2 |> List.iter compareTyping
        compareTypeExpression (typeExpr1, typeExpr2)
...
let compareTransitionR (ts1, ts2) =
    match ts1, ts2 with
    | Variable vd1, Variable vd2 -> List.zip vd1 vd2 |> List.iter compareValueDeclaration
    | InitConstraint ve1, InitConstraint ve2 -> List.zip ve1 ve2 |> List.iter compareValueExpression
    | TransitionRule(valueExpr1, l1), TransitionRule(valueExpr2, l2) ->
        ...
        List.zip l1 l2 |> List.iter (fun ((_, e1), (_, e2)) -> compareValueExpression (e1, e2))
...
let compareDeclaration dec1 dec2 =
    ...
    | Value v1, Value v2 ->
        ...
        List.zip v1 v2 |> List.iter compareValueDeclaration
    | TypeDeclaration td1, TypeDeclaration td2 ->
        Assert.AreEqual(List.length td1, List.length td2)
        List.zip td1 td2 |> List.iter compareTypeDefinition
    | AxiomDeclaration ad1, AxiomDeclaration ad2 -> List.zip ad1 ad2 |> List.iter compareValueExpression
    | TransitionSystemDeclaration((s1, _), transitionSystems1), TransitionSystemDeclaration ((s2, _), transitionSystems2) ->
        Assert.AreEqual(s1, s2)
        List.zip transitionSystems1 transitionSystems2 |> List.iter compareTransitionR
...
let compareClass class1 class2 =
    ...
    List.zip class1 class2 |> List.iter (fun (e1, e2) -> compareDeclaration e1 e2)
