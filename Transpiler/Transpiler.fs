...
let buildValueTable (_AST: Class) =
    ...
    let unfoldValueEnvironments acc = function
        | Value v -> v@acc
        | _ -> acc
    
    let unfoldValueValues (map: Map<string, TypeExpression>) = function
...
let rec unfoldValueExpr typeEnv valueEnv =
    ...
    | Quantified(_, tl, ve) as qq ->
        ...
        | Equivalence (GenericName (name, _), value_expr) ->
            List.foldBack (fun e acc -> ExplicitValue($"{name}{e}", (Map.find name valueEnv), value_expr)::acc) prefixes []
...
let unfoldDeclaration typeEnv valueEnv decl =
    ...
    | TypeDeclaration typeDeclarations ->
        TypeDeclaration(List.foldBack (fun typeDecl acc -> (unfoldType typeEnv valueEnv typeDecl) @ acc) typeDeclarations [])
    | AxiomDeclaration axioms -> Value (List.foldBack (fun e acc -> (unfoldValueExpr typeEnv valueEnv e) @ acc) axioms [])
